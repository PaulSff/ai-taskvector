"""
Compact diff between two graphs (units + connections). No dependency on agents.
...
def graph_diff(
    ...
    if prev is None or current is None:
        return ""
    prev_d = (
        prev.model_dump(by_alias=True) if isinstance(prev, ProcessGraph) else dict(prev)
    )
    curr_d = (
    ...
    if added_units:
        parts.append(
            "added "
            + ", ".join(f"{uid} ({utype})" for uid, utype in sorted(added_units))
        )
    ...
    if added_conns:
        parts.append(
            "connected " + ", ".join(f"{a}->{b}" for a, b in sorted(added_conns))
        )
    removed_conns = prev_conns - curr_conns
    if removed_conns:
        parts.append(
            "disconnected " + ", ".join(f"{a}->{b}" for a, b in sorted(removed_conns))
        )
