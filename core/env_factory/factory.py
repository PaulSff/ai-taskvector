...
def build_env(
    ...
    **kwargs: Any,
    ...
    if process_graph.environment_type == EnvironmentType.MESSENGERS:
        from environments.graph_env import GraphEnv
        from environments.native.messengers import MessengersEnvironmentSpec

        spec = MessengersEnvironmentSpec()
        return GraphEnv(
            process_graph,
            goal,
            spec,
    ...
    if process_graph.environment_type == EnvironmentType.NETWORK:
        from environments.graph_env import GraphEnv
        from environments.native.network import NetworkEnvironmentSpec

        spec = NetworkEnvironmentSpec()
        return GraphEnv(
            process_graph,
            goal,
            spec,
    ...
    if process_graph.environment_type == EnvironmentType.DISCOVERY:
        from environments.graph_env import GraphEnv
        from environments.native.discovery import DiscoveryEnvironmentSpec

        spec = DiscoveryEnvironmentSpec()
        return GraphEnv(
            process_graph,
            goal,
            spec,
    ...
    if process_graph.environment_type == EnvironmentType.CODING:
            from environments.graph_env import GraphEnv
            from environments.native.coding import CodingEnvironmentSpec

            spec = CodingEnvironmentSpec()
            return GraphEnv(
                process_graph,
                goal,
                spec,
    ...
    raise ValueError(
        ...
        f"Supported: thermodynamic, data_bi, web, messengers, time, network, discovery, semantics, rag, office, taskvector, coding."
    )
