"""Receptionist agents chat turn: same pipeline as Workflow Designer without structural graph edits."""
...
from agents.tools.catalog import ORDERED_RECEPTIONIST_TOOLS, receptionist_tool_ids
...
class ReceptionistChatHandler:
    """Runs receptionist_workflow.json: tools + comments/todos only (no structural graph edits)."""

    @property
    def role_id(self) -> str:
        return RECEPTIONIST_ROLE_ID

    @property
    def role_name(self) -> str:
        return get_role(RECEPTIONIST_ROLE_ID).role_name
    ...
    async def run_turn(
        ...
        overrides = build_agent_workflow_unit_param_overrides(
            ...
            prompt_template_path=_RECEPTIONIST_PROMPT_PATH,
            llm_options_role_id=RECEPTIONIST_ROLE_ID,
            rag_top_k_role_id=RECEPTIONIST_ROLE_ID,
        )
        ...
        _an_role = get_role(RECEPTIONIST_ROLE_ID)
        max_follow_ups = (
        ...
        follow_up_tools = (
            _an_role.tools if _an_role.tools else tuple(receptionist_tool_ids())
        )
        ...
        async def _parser_output_follow_up_chain(
            ...
            parser_ctx = ParserFollowUpContext(
                ...
                agent_role_id=RECEPTIONIST_ROLE_ID,
                agent_workflow_path=_RECEPTIONIST_WORKFLOW_PATH,
                analyst_mode=True,
                ordered_follow_up_tools=ORDERED_RECEPTIONIST_TOOLS,
        ...
        try:
            ...
            response = await turn_ctx.run_workflow_streaming(
                ...
                workflow_path=_RECEPTIONIST_WORKFLOW_PATH,
            )
