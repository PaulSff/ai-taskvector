...
1. **Resolve workflow path** from `edit["action"]` (e.g. `"add_unit"` → `gui/components/workflow_tab/edit_workflows/edit_add_unit.json`). For **`add_comment`** and **todo** actions, use `runner._edit_workflow_path` (delegates to `get_tool_workflow_path` for `add_comment` / `todo_manager`).
...
EDIT_WORKFLOWS_DIR = Path(__file__).parent  # gui/components/workflow_tab/edit_workflows
