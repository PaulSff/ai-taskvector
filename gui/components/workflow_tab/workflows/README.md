...
| **workflows/edit_workflows/** | One workflow per graph-edit action (add_unit, connect, disconnect, etc.). Used by the edit runner when applying assistant edits. **add_comment** and **todo_list** edit graphs are loaded from `assistants/tools/add_comment/` and `assistants/tools/todo_manager/` via `tool.yaml`. | `edit_add_unit.json`, `edit_connect.json`, … |
| **workflows/assistants_workflows/** | Chat/assistant helpers: units library, etc. Client todo injection graph lives under `assistants/tools/todo_manager/` (`tool.yaml` + `todo_list.json`). RAG context: `assistants/tools/rag_search/`. RAG index/doc graphs: **`rag/workflows/`** (`rag_update.json`, `doc_to_text.json`). | `units_library_workflow.json` |
