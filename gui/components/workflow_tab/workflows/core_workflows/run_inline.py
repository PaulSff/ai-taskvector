...
JSON files in this package (under ``workflows/core_workflows/``) and sibling ``workflows/agents_workflows/`` are required; there is no fallback to
...
def _run(
    ...
    return run_workflow(
        ...
        format="dict",
        execution_timeout_s=EXECUTION_TIMEOUT_S,  # choose a value appropriate for your system
    )
...
def run_graph_summary_inline(graph: Any) -> dict[str, Any]:
...
def run_units_library_source_paths_inline(
...
def run_graph_diff_inline(prev_graph: Any, current_graph: Any) -> str | None:
...
def run_export_workflow_inline(graph: Any, format: str) -> tuple[Any, str | None]:
...
def run_runtime_label_inline(graph: Any) -> tuple[str, bool]:
...
def run_apply_training_config_edits_inline(
    training_config: Any,
    edits: list[dict[str, Any]],
) -> tuple[dict[str, Any] | None, str | None]:
...
def run_normalize_graph_inline(
...
async def validate_graph_to_apply_for_canvas_inline(
    ...
    Use from Flet chat instead of ``ProcessGraph.model_validate`` on raw agent output dicts.
    """
    if graph is None:
    ...
    err = unit_out.get("error")
    if err:
        print("validate_graph_to_apply out err:", err)
        return (None, str(err))
...
def run_clean_text_for_chat_inline(text: str) -> str:
    """
    ...
    Uses ``agents_workflows/clean_text_chat_single.json`` (max_chars=0, min_block_len=1). Callers avoid importing
