...
Workflow definitions (`.json` DAGs) under `gui/flet/components/workflow/` are grouped by purpose (RAG helper graphs additionally live under **`rag/workflows/`** at repo root):
...
| **assistants/** | Chat/assistant helpers: units library, etc. Client todo injection graph lives under `assistants/tools/todo_manager/` (`tool.yaml` + `todo_list.json`). RAG context: `assistants/tools/rag_search/`. RAG index/doc graphs: **`rag/workflows/`** (`rag_update.json`, `doc_to_text.json`). | `units_library_workflow.json` |
...
Most paths used in code (e.g. `core_workflows.py`, `todo_list_manager.py`) point into these folders; RAG index/doc helper graphs live under **`rag/workflows/`** (see `rag_update_workflow_path` / `doc_to_text_workflow_path` in **`rag/ragconf.yaml`**).
