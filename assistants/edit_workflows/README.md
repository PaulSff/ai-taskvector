...
Each JSON file defines a minimal process graph: **Inject** → one edit unit (add_unit, connect, disconnect, etc.). The assistant backend runs the matching workflow per `GraphEdit.action`, injecting the current graph and the edit params, and uses the edit unit’s output as the updated graph for the GUI.
