...
The **RL Coach** is the TaskVector assistant for **RL training configuration**: goals, reward design (presets, formulas, rules), algorithm choice, and hyperparameters. It proposes or applies **declarative edits** to the training config (YAML/JSON). It does **not** edit `runtime/train.py` or hand-written reward Python; the training runtime reads the merged config file.

| Item | Location |
|------|----------|
| Role config (`tools`, `chat`, LLM defaults) | `assistants/roles/rl_coach/role.yaml` |
| Chat process graph (canonical JSON) | `assistants/roles/rl_coach/rl_coach_workflow.json` |
| `initial_inputs` builders (headless-friendly) | `assistants/roles/rl_coach/workflow_inputs.py` — `build_rl_coach_initial_inputs`, `build_rl_coach_training_inject_updates`, `build_rl_coach_assistant_aligned_initial_inputs` (WD-aligned graph + training injects) |
| Default system prompt text | `assistants/prompts.py` → `RL_COACH_SYSTEM`; template path often `config/prompts/rl_coach.json` (overridable via settings / `build_rl_coach_unit_param_overrides`) |
...
| Shared JSON runner | `gui/chat/assistant_workflow/run.py` → `run_assistant_workflow()` |

Resolve the workflow file with `assistants.roles.workflow_path.get_role_chat_workflow_path` (`RL_COACH_ROLE_ID` / `role.yaml` `chat.workflow`).
...
**Full chat parity with Workflow Designer graph fields** (`build_rl_coach_assistant_aligned_initial_inputs`): starts from `build_assistant_workflow_initial_inputs` (graph, follow-up, language, …) then **`update(..., build_rl_coach_training_inject_updates(...))`** so the same RAG + merge path can run with training ports filled.

Rule: each **Inject** id maps to `initial_inputs[id] = {"data": value}` when calling `run_workflow` / `run_assistant_workflow`.
...
`run_rl_coach_workflow` delegates to **`run_assistant_workflow(..., workflow_path=RL_COACH_WORKFLOW_PATH)`**.

### CLI

```bash
python -m runtime assistants/roles/rl_coach/rl_coach_workflow.json --format dict --initial-inputs @inputs.json
```
