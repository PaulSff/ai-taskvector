...
| Build initial_inputs | (1) **Merge** inputs: one Inject per source (user_message, graph_summary, turn_state, recent_changes_block, last_edit_block). units_library from **UnitsLibrary** (inject_graph_summary → UnitsLibrary → Merge). rag_context from **RagSearch → Filter (data_bi) → FormatRagPrompt** (inject_user_message → rag_search → rag_filter → format_rag → Merge); pass unit_param_overrides["rag_search"] = {persist_dir, embedding_model}. (2) **Process** graph: initial_inputs["inject_graph"] = {"data": current_graph}. |
