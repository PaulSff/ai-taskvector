...
- **agents/roles/workflow_designer/README.md** — Workflow Designer role: graph overview, `initial_inputs`, and running `workflow_designer_workflow.json` via `run_workflow()` / `run_agent_workflow()`.
- **agents/roles/rl_coach/README.md** — RL Coach role: training injects, reward/config merge, and running `rl_coach_workflow.json` via `run_workflow()` / `run_rl_coach_workflow()`.
