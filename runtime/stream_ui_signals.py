...
CHAMELEON_STREAM_PREFIX = "\xff\xfeflet_chameleon_stream\xff\xfe"
...
def chameleon_stream_chunk(payload: dict[str, Any]) -> str:
    """Structured Chameleon partial/final step payload on the same callback as tokens (parse by prefix)."""
    return CHAMELEON_STREAM_PREFIX + json.dumps(payload, default=str, separators=(",", ":"))
