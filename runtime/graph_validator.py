...
def validate_graph_for_execution(graph: ProcessGraph) -> None:
    ...
    for c in graph.connections:
        ...
        if c.from_id in process_ids and not from_unit.output_ports:
            raise ValueError(
                f"Connection from unit '{c.from_id}' has no output_ports; every process unit used as a connection source must have output_ports on the graph."
            )
        if c.to_id in process_ids and not to_unit.input_ports:
            raise ValueError(
                f"Connection to unit '{c.to_id}' has no input_ports; every process unit used as a connection target must have input_ports on the graph."
            )
        if c.from_id in process_ids and from_unit.output_ports:
            ...
            if fp < 0 or fp >= len(from_unit.output_ports):
                raise ValueError(
                    f"Connection from_port '{c.from_port}' out of range for unit '{c.from_id}' (has {len(from_unit.output_ports)} output_ports)."
                )
        if c.to_id in process_ids and to_unit.input_ports:
            ...
            if tp < 0 or tp >= len(to_unit.input_ports):
                raise ValueError(
                    f"Connection to_port '{c.to_port}' out of range for unit '{c.to_id}' (has {len(to_unit.input_ports)} input_ports)."
                )
