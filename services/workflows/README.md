...
Most paths used in code (e.g. `workflows.core_workflows` package, `agents/chat/context/todo_list_manager.py`) point into these folders; RAG index/doc helper graphs live under **`rag/workflows/`** (see `rag_update_workflow_path` / `doc_to_text_workflow_path` in **`rag/ragconf.yaml`**).
