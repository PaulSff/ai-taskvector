...
- **Params:** `max_chars` and `snippet_max` — each may be:
  ...
  - a **param ref** string resolved by `units.canonical.app_settings_param`: `settings.<key>` (`config/app_settings.json`), `tool.<id>.<dotted.path>` (`agents/tools/<id>/tool.yaml`), or `role.<id>.<dotted.path>` (`agents/roles/<id>/role.yaml`). RAG defaults use `tool.rag_search.rag.*`; read_file overrides use `tool.read_file.rag.*`.
