...
- Uses the formulas library (https://pypi.org/project/formulas/) APIs commonly found across versions:
    - Read workbook: `ExcelCompiler().read(path)` or equivalent `Parser.read/loads`.
    - Optional compile step: compiler.compile(workbook) — used if present ("if required" for some formulas versions).
    - Evaluate/recalculate: `workbook.recalculate()` / `workbook.evaluate()` / `compiler.evaluate_all(workbook)`.
- Read/write cells: `sheet.cell(...)`, `sheet.cells[(row,col)]`, `cell.value` / `cell.result` / `cell.raw_value`.
