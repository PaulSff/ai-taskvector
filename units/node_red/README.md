...
Built-in Node-RED node types as an **environment** in our units system. Same idea as `units/pyflow`: the agent can add units from this catalog when the graph has environment `node_red`; they are stored in canonical form and get a JavaScript `code_block` for export (no JS executor in-app yet).
...
- **add_unit**: When the agent adds a unit whose type is in the catalog and the graph origin is node_red, `graph_edits` attaches the JS template as the unit’s code_block (for export to Node-RED).
