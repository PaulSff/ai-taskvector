...
| **units** | `list[dict]` | One row per requested id: **`unit_id`**, **`found`** (unit exists in `graph["units"]`), **`has_code_block`** (that id appears in **code_block_ids**), **`unit_type_raw`**, **`unit_type_canonical`** (after `canonical_unit_type`). |
