...
Uses `core.gym.training_edits.apply_config_edits` and `core.normalizer.to_training_config`. Used in `agents/roles/rl_coach/rl_coach_workflow.json`: inject_training_config_dict → ApplyTrainingConfigEdits (training_config); TrainingConfigParser (edits) → ApplyTrainingConfigEdits (edits). Standalone workflow: `gui/components/workflow_tab/workflows/core_workflows/apply_training_config_edits_single.json` (see `gui.components.workflow_tab.workflows.core_workflows.run_apply_training_config_edits`).
