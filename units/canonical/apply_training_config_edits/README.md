...
Uses `core.gym.training_edits.apply_config_edits` and `core.normalizer.to_training_config`. Used in `rl_coach_workflow.json`: inject_training_config_dict → ApplyTrainingConfigEdits (training_config); TrainingConfigParser (edits) → ApplyTrainingConfigEdits (edits).
