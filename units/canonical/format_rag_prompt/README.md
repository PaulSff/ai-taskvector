...
- **Output:** `data` (str) — the block to inject as `rag_context` in Merge/Prompt.
...
If either param is missing or invalid, the unit outputs empty string. Flow: **RagSearch → Filter (data_bi) → FormatRagPrompt → Merge** (rag_context key).
