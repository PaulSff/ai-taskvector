...
Used in the agent workflow: **inject_graph_summary → UnitsLibrary → Merge** (units_library key). Callers no longer inject `units_library` manually; the unit computes it from `graph_summary` at run time.
