...
Used in the agent workflow so the runner injects only the graph; the workflow produces the summary and feeds it to Merge (graph_summary key) and UnitsLibrary. Replaces the need for a separate `inject_graph_summary` inject.
