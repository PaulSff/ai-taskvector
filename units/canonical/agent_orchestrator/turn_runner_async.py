...
async def run_orchestrator_turn_async(
    ...
    *,
    ...
    # ── Run main workflow (run_agent_workflow is sync: use threadpool) ──
    ...
    try:
        ...
        response = await _parser_chain_runner_async(response)
        ...
        # canonicalize_add_comment_edits may be sync; run in thread
        ...
        if result.get("kind") == "applied" and result.get("graph") is not None:
            # Apply/augment graph (sync -> thread)
            applied_graph, _supplements, _v_err = await asyncio.to_thread(
                _apply_and_augment_graph,
                result["graph"],
                result.get("edits") or [],
                {"coding_is_allowed": coding_is_allowed},
        ...
        # ── Handle apply_failed ──
            ...
            (
                ...
                retry_result,
                retry_content,
            ) = await _run_self_correction_retry_async(
                failed_apply,
                session,
                role_config,
                graph_ref,
