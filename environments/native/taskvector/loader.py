"""
AI Taskvector env loader: build from process_graph + goal via env_factory.
Python-only units.
"""
...
def load_taskvector_env(
    ...
    **kwargs: Any,
    ...
    """Build taskvector env from process graph + goal (delegate to env_factory)."""
    if process_graph is None:
        path = config.get("process_graph_path")
        if not path:
            raise ValueError(
                "Taskvector config must include 'process_graph_path' or pass process_graph"
            )
