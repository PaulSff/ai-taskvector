"""
Discovery env loader: build from process_graph + goal via env_factory.
Python-only units.
"""
...
def load_discovery_env(
    ...
    **kwargs: Any,
    ...
    """Build discovery env from process graph + goal (delegate to env_factory)."""
    if process_graph is None:
        path = config.get("process_graph_path")
        if not path:
            raise ValueError(
                "Discovery config must include 'process_graph_path' or pass process_graph"
            )
