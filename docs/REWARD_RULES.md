...
For **scripted** merges without the chat UI, load the YAML into a config object (or dict), build the edit dict, then call `run_apply_training_config_edits(config, [edit])` from `gui.flet.components.workflow.core_workflows` (same **ApplyTrainingConfigEdits** unit as `rl_coach_workflow.json`). Alternatively use `apply_training_config_edit` from `edit_workflows/training_edit_runner.py` for direct merge + normalize without a workflow run.
