...
**Universal RLOracle (template-based injection):** `deploy.inject_oracle_into_flow()` (Node-RED), `deploy.inject_oracle_into_n8n_flow()` (n8n), `deploy.inject_oracle_into_comfyui_workflow()` (ComfyUI), and `deploy.inject_oracle_into_process_graph()` add a standard Oracle from **adapter_config** only — environment-agnostic, no embedded simulations. (In-graph training with our simulator uses canonical topology by **role** — step_driver, join, switch — from the unit registry; deploy injects the external-runtime RLOracle, not those in-graph units.) All parameters come from the training config: `observation_spec`, `action_spec`, `reward_config`, `max_steps`. The Oracle is a thin protocol layer: step driver (reset/step routing) + collector (build obs, compute reward from `reward_config`). Convention: sensors send to collector with `topic=observation_name`; process writes actions from `flow.get('action')`. n8n uses Webhook + Code nodes with `$getWorkflowStaticData()`. **PyFlow (in-process):** RLOracle units with Python code_blocks. **ComfyUI:** custom nodes + bridge expose `/step`. See **deploy/oracle_inject.py**, **deploy/README.md**, and **deploy/templates/**.
