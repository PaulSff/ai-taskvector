...
**RLOracle (canonical only):** Add RLOracle via **add_pipeline** (type `"RLOracle"`) in the graph; this creates the canonical topology (step_driver, step_rewards, join, switch, etc.) and attaches Oracle code to the canonical units. Export to Node-RED, n8n, or PyFlow then produces runnable flows. All parameters from **adapter_config**: `observation_spec`, `action_spec`, `reward_config`, `max_steps`. Convention: sensors → join → step_rewards; step_driver routes reset/step; process writes actions from `flow.get('action')`. See **deploy/oracle_inject.py** (`render_oracle_code_blocks_for_canonical`), **deploy/README.md**, and **units/canonical/PIPELINES-WIRING.md**.
