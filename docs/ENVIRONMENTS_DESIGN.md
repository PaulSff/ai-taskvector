...
**Node-RED runtime as external env:** Node-RED can also be the external environment (sensors in, actions out); training would use its runtime I/O. Adapter stub: `environments/external/node_red_adapter.py` (wrap Node-RED as gym.Env via `BaseExternalWrapper`). **EdgeLinkd** ([Node-RED reimplemented in Rust](https://github.com/oldrev/edgelinkd)) is supported as an alternative runtime for faster execution via `environments/external/node_red_rust_edgelinkd_adapter.py`. Deployment: trained model as a custom Node-RED node — see **docs/DEPLOYMENT_NODERED.md**.
