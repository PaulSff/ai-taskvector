...
| **Assistants** | Process (graph edits) + Training (config edits); apply → normalizer → canonical | **Done.** `assistants/` (graph_edits, config_edits, process_assistant, training_assistant); text-to-reward (Ollama); CLI apply_graph/apply_config. |
