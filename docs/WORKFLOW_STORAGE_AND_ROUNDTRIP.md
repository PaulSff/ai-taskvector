# Workflow storage format and roundtrip (Node-RED, PyFlow, n8n, ComfyUI, canonical)
...
| **n8n** | Normalized via `to_process_graph(raw, format="n8n")` | **ProcessGraph** (+ optional layout + optional origin blob) |
| **ComfyUI** | Normalized via `to_process_graph(raw, format="comfyui")` | **ProcessGraph** (+ optional layout + optional origin blob) |
...
- **Roundtrip:** To re-export to Node-RED, PyFlow, n8n, or ComfyUI we either:
...
1. **Accept Node-RED, PyFlow, and our graph on import**  
   Already supported in the import dialog (`to_process_graph(..., format="node_red"|"pyflow"|"n8n"|"comfyui"|"dict")`). All paths produce the same **ProcessGraph**; no extra “format” to store beyond optional `origin_format` + `origin_blob` if we want lossless re-export.
...
2. **Preview code of all those workflows in the editor**  
   ...
   So we need at least one of: store `origin_blob`, or implement canonical → node_red / pyflow / n8n / comfyui serializers. Preview could offer a dropdown: "View as: Canonical | Node-RED | PyFlow | n8n | ComfyUI".
...
**Store everything as ProcessGraph (canonical); add optional `layout` (positions per unit_id) and optional `origin_format` + `origin_blob` for roundtrip; use layout on import from Node-RED/PyFlow/n8n/ComfyUI and when saving after drag; implement preview by either keeping origin_blob or adding inverse normalizers (canonical → node_red / pyflow / n8n / comfyui).**
