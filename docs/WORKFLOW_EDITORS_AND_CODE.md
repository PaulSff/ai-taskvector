...
| **n8n** | Yes. Workflow export as JSON (Editor → Download / Import from File). Structure: `nodes` (array with `id`, `name`, `type`, `typeVersion`, `position`, `parameters`), `connections` (object keyed by **node name**). Code node: `n8n-nodes-base.code` with `parameters.jsCode`. | Yes | `n8n` |
| **ComfyUI** | Yes. Workflow v1.0: `nodes` (id, type, pos, size, inputs, outputs, widgets_values), `links` (id, origin_id, origin_slot, target_id, target_slot). | Yes | `comfyui` |
...
**Usage:** Load a JSON file with the right format: `to_process_graph(raw, format="node_red"|"pyflow"|"ryven"|"idaes"|"n8n"|"comfyui")` (for `.json` we default to `node_red`; pass the format explicitly when needed).
...
| **n8n** | — | **Import + deploy** | Training: expose step API (webhook + Code nodes). Deploy: `inject_agent_into_n8n_flow`. RLOracle: add_pipeline + export. | Import + deploy; full RLOracle support. |
| **ComfyUI** | `environments/external/comfyui_adapter.py` | **Implemented** | **Yes.** Bridge exposes `/step`; flow runs in ComfyUI. Deploy: `inject_agent_into_comfyui_workflow`. RLOracle: build graph with add_pipeline, export to ComfyUI. Custom nodes: `deploy/custom_nodes/ai_control_rl/`. | Import + train + deploy. |

**Import side:** Normalizer supports Node-RED, PyFlow, Ryven, IDAES, **n8n**, and **ComfyUI**. For IDAES: use format `idaes` with a template-like dict (`units`/`blocks` + `connections`/`links`). **Runtime side:** Node-RED (HTTP/WebSocket), PyFlow (in-process), Ryven (HTTP/WebSocket), IDAES (in-process), and **ComfyUI** (HTTP via bridge) adapters are implemented. **Deploy:** `inject_agent_into_flow` (Node-RED), `inject_agent_into_pyflow_flow` (PyFlow), `inject_agent_into_n8n_flow` (n8n), `inject_agent_into_comfyui_workflow` (ComfyUI).
...
| **Are runtime adapters ready?** | Yes. Node-RED (HTTP/WebSocket), PyFlow (in-process), Ryven (HTTP/WebSocket), IDAES (in-process), ComfyUI (HTTP via bridge) are implemented. n8n: full import + deploy + Oracle. Deploy: `inject_agent_into_flow` (Node-RED), `inject_agent_into_pyflow_flow` (PyFlow), `inject_agent_into_n8n_flow` (n8n), `inject_agent_into_comfyui_workflow` (ComfyUI). See **deploy/README.md** for training vs inference (RLOracle/step vs RLAgent/predict). |
