...
| **Are runtime adapters ready?** | Yes. Node-RED (HTTP/WebSocket), PyFlow (in-process), Ryven (HTTP/WebSocket, default port 1899), and IDAES (in-process) are implemented. Deploy: `inject_agent_into_flow` (Node-RED/EdgeLinkd) and `inject_agent_into_pyflow_flow` (PyFlow). IDAES: use trained model in control/optimization (no flow-inject node yet). |
