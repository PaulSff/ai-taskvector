...
| **n8n** | — | **Import + deploy** | Training: expose step API (webhook + Code nodes). Deploy: `inject_agent_into_n8n_flow`, `inject_oracle_into_n8n_flow`. | Import + deploy; full RLOracle support. |
| **ComfyUI** | `environments/external/comfyui_adapter.py` | **Implemented** | **Yes.** Bridge exposes `/step`; flow runs in ComfyUI. Deploy: `inject_agent_into_comfyui_workflow`, `inject_oracle_into_comfyui_workflow`. Custom nodes: `deploy/custom_nodes/ai_control_rl/`. | Import + train + deploy. |
