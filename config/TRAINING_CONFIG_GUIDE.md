...
Natural language → reward edits are done in-app via the **RL Coach**. To merge a hand-written JSON edit from scripts, use `gui.components.workflow_tab.core_workflows.run_apply_training_config_edits` (see `scripts/test_assistants.py`) or `runtime.run.run_workflow` on `gui/components/workflow_tab/core_workflows/apply_training_config_edits_single.json`.
