"""Load per-tool action prompt lines from ``agents/tools/<tool_id>/prompt.py`` (no package __init__).

Importing ``agents.tools.<tool_id>`` would run subpackage ``__init__.py`` files that depend on the GUI
and ``agents.prompts``; workflow designer system prompt expansion runs during ``agents.prompts``
...
def get_tool_action_prompt_line(tool_id: str) -> str:
    ...
    name = f"_agents_tools_{tool_id}_prompt"
