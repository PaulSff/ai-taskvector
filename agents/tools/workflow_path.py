"""Resolve workflow JSON paths from ``agents/tools/<tool_id>/tool.yaml`` ``workflow`` field."""
...
def get_tool_workflow_path(tool_id: str) -> Path:
    ...
    - Reads ``workflow`` from ``agents/tools/<tool_id>/tool.yaml``.
    - If relative and starts with ``agents/``, ``gui/``, ``config/``, or ``rag/``: resolve from repo root.
    - If relative otherwise: resolve under ``agents/tools/<tool_id>/``.
    ...
    if not raw:
        raise ValueError(
            f"tool.yaml for {key!r} must set ``workflow`` (workflow JSON path)."
        )
    p = Path(raw).expanduser()
    if p.is_absolute():
        return p.resolve()
    norm = str(p).replace("\\", "/")
    if norm.startswith(("agents/", "gui/", "config/", "rag/")):
