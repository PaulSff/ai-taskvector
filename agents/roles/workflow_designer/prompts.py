...
Canonical location: ``agents/roles/workflow_designer/prompts.py``. Re-exported from ``agents.prompts`` for stable imports.

Role-specific ``config/prompts/workflow_designer.json`` ``fragments`` keys are applied by
``apply_workflow_designer_role_fragments`` (called from ``agents.prompts`` after this module loads).

Per-tool JSON action lines for the "Extra actions" section live in ``agents/tools/<tool_id>/prompt.py``
...
from agents.tools.add_comment.follow_ups import (
...
from agents.tools.import_workflow.follow_ups import (
...
# Workflow Designer (process graph edits): "Environment / Process agent"
#
...
#      Environment: If the graph has no environments (missing or empty), only canonical and environment-agnostic units are shown. To get env-specific units, the agent must first add an environment using add_environment (e.g. {"action":"add_environment","env_id":"thermodynamic"}). When the graph has environments set, units whose tags match and env-agnostic types are shown.
...
#      When: Re-run after the agent requested search/file/browse/code_block; chat fetches content and re-runs with inject_follow_up_context.
...
# (and shared empty-tool lines in ``agents/tools/follow_up_common.py``).
# Optional overrides: ``config/prompts/workflow_designer.json`` ``fragments`` keys → see
# ``agents/tools/follow_up_fragment_overrides.py``.

# Import post-apply strings: canonical text in ``agents/tools/import_workflow/follow_ups.py`` (aliases above).
# Default post-apply strings: canonical in ``agents/follow_ups.py`` (aliases for fragment keys / star export).
...
def apply_workflow_designer_role_fragments(fragments: dict[str, Any]) -> None:
    ...
    if not fragments:
        return
    import agents.follow_ups as _shared_fu
