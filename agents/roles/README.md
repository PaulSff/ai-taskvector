...
introduction_words: |  # optional: opening self-intro for the system prompt (falls back to name + role_name)
  My name is Alex. I am the My Role agent at TaskVector.
...
# Optional: main Flet agents chat (`list_chat_dropdown_role_ids`, `RoleConfig.chat`).
# chat:
#   enabled: true
...
Rules enforced by the loader (`agents/roles/registry.py`):
...
Put agent-specific workflow JSON next to the role when the product expects it, for example:
