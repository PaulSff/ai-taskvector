...
Canonical location: ``agents/roles/rl_coach/prompts.py``. Re-exported from ``agents.prompts``.

Per-tool JSON lines use ``{tool:tool_id}`` placeholders, expanded at import by
``agents.tools.prompt_lines.expand_tool_action_placeholders`` (same pattern as Analyst /
Workflow Designer). Order matches ``agents/tools/catalog.py`` ``ORDERED_RL_COACH_TOOLS``.
"""

from agents.tools.prompt_lines import expand_tool_action_placeholders

# RL Coach (training config edits): "Training agent"
...
RL_COACH_SYSTEM = (
    f"{_rl_coach_introduction_block()}\n\n" + _RL_COACH_SYSTEM_BODY_EXPANDED
)
