"""Planner agents chat turn (extracted from ``chat.py``).
...
_IS_LIGHT_GRAPH_MODE_ENABLED = is_role_light_graph_mode_enabled(PLANNER_ROLE_ID)
...
class PlannerChatHandler:
    ...
    async def run_turn(
        ...
        *,
        ...
        wd_role = get_role(PLANNER_ROLE_ID)
        max_wd_follow_ups = (
        ...
        wd_follow_up_tools = (
            ...
            if wd_role.tools
                ...
                tid
                for tid, _ in ordered_tools_for_role_id(PLANNER_ROLE_ID)
            )
        )
