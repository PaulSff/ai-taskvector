"""Planner agents chat turn: same pipeline as Workflow Designer without structural graph edits."""
...
from agents.roles import PLANNER_ROLE_ID, get_role
...
_PLANNER_WORKFLOW_PATH = get_role_chat_workflow_path(PLANNER_ROLE_ID).resolve()
...
class PlannerChatHandler:
    ...
    def role_name(self) -> str:
        return get_role(PLANNER_ROLE_ID).role_name
    ...
    async def run_turn(
        ...
        overrides = build_agent_workflow_unit_param_overrides(
            ...
            prompt_template_path=_PLANNER_PROMPT_PATH,
            llm_options_role_id=PLANNER_ROLE_ID,
            rag_top_k_role_id=PLANNER_ROLE_ID,
        )
        ...
        _an_role = get_role(PLANNER_ROLE_ID)
        max_follow_ups = (
        ...
        follow_up_tools = _an_role.tools if _an_role.tools else tuple(
            tid for tid, _ in _ordered_tools_for_role_id(PLANNER_ROLE_ID)
        )
        ...
        async def _parser_output_follow_up_chain(
            ...
            parser_ctx = ParserFollowUpContext(
                ...
                agent_role_id=PLANNER_ROLE_ID,
                agent_workflow_path=_PLANNER_WORKFLOW_PATH,
                analyst_mode=True,
                ordered_follow_up_tools=_ordered_tools_for_role_id(PLANNER_ROLE_ID),
        ...
        try:
            ...
            response = await turn_ctx.run_workflow_streaming(
                ...
                workflow_path=_PLANNER_WORKFLOW_PATH,
            )
