...
class ReceptionistChatHandler:
    ...
    async def run_turn(
        ...
        *,
        ...
        receptionist_role = get_role(self.role_id)

        max_follow_ups = (
            receptionist_role.follow_up_max_rounds
            if receptionist_role.follow_up_max_rounds is not None
            else get_workflow_designer_max_follow_ups()
        )

        follow_up_tools = (
            receptionist_role.tools
            if receptionist_role.tools
            else tuple(
                tool_id
